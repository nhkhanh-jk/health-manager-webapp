...
const Topbar = () => {
  ...
  return (
    <>
      <nav
        className="glass-topbar sticky top-4 z-30 ml-4 mr-4 lg:ml-6 lg:mr-8 rounded-2xl shadow-sm"
        style={{
          boxShadow:
      ...
      >
        <div className="max-w-5xl mx-auto px-4 lg:px-6 py-3">
          <div className="flex items-center justify-between gap-2 lg:gap-4">
            ...
            <button
              className="md:hidden p-2.5 rounded-xl glass-button hover:bg-[var(--glass-bg-hover)] text-[var(--text-secondary)] transition-colors"
              onClick={() => {
                // You can add a mobile search modal here if needed
                const query = prompt(t("search"));
                if (query) {
                  performSearch(query);
            ...
            <div className="flex items-center space-x-2 lg:space-x-3">
              ...
              <div className="relative" ref={userMenuRef}>
                <motion.button
                  className="flex items-center space-x-2 lg:space-x-3 p-1.5 lg:p-2 rounded-xl glass-button hover:bg-[var(--glass-bg-hover)] transition-colors"
                ...
                >
                  <div className="w-8 h-8 lg:w-9 lg:h-9 rounded-full bg-gradient-to-br from-[var(--primary-600)] to-[var(--accent-600)] flex items-center justify-center text-white font-semibold text-xs lg:text-sm">
      ...
      <AnimatePresence>
        {showNotifications && (
          ...
          >
            <motion.div
              className="absolute w-[calc(100vw-2rem)] md:w-96 glass-modal shadow-xl max-h-[80vh] overflow-hidden flex flex-col"
              style={{
              ...
              <div className="flex-1 overflow-y-auto">
                {notifList.length === 0 ? (
                  ...
                  notifList.map((notif) => (
                    <div
                      key={notif.id}
                      className={`p-4 border-b border-[var(--neutral-200)] cursor-pointer ${!notif.read ? "bg-[var(--primary-50)]" : ""
                        }`}
                      onClick={() => {
                        markAsRead(notif.id);
                        if (notif.link) {
                      ...
                      <div className="flex items-start justify-between">
                        <div className="flex-1 pr-2">
                          <div className="flex items-center space-x-2 mb-1">
                            <span
                              className={`w-2 h-2 rounded-full flex-shrink-0 ${notif.type === "success"
      ...
      <AnimatePresence>
        {showUserMenu && (
          ...
          >
            <motion.div
              className="absolute w-[calc(100vw-2rem)] md:w-56 glass-modal shadow-xl"
              style={{
