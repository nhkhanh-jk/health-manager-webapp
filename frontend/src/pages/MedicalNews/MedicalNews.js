...
const MedicalNews = () => {
    ...
    const fetchNews = async () => {
      try {
        ...
        const response = await api.get("/news", {
          params: {
            q: searchTerm || 'health',
            category: selectedCategory, // --- MỚI: Thêm category để backend map query ---
            limit: 20 // Backend dùng 'limit' hoặc 'pageSize' đều được
          },
        });
        // --- HẾT SỬA LỖI ---
  ...
  const NewsCard = ({ news }) => (
    <motion.div
      className={`p-6 rounded-xl shadow-lg border cursor-pointer transition-all duration-300 ${theme === "dark"
  ...
  return (
    ...
    >
      ...
      {loading ? (
        ...
        <div
          className={`p-8 rounded-xl text-center border ${theme === "dark"
              ? "bg-[var(--glass-bg-primary)] border-[var(--glass-border)]"
              : "bg-white border-gray-200"
            }`}
        >
