...
const NewChatbot = () => {
  ...
  const [messages, setMessages] = useState(() => {
    // Nếu chưa có user ID thì chưa load (tránh lỗi khi chưa login xong)
    if (!user?.id) return [];
    
    // Tạo key riêng cho từng user để không bị lẫn tin nhắn
    const storageKey = `chat_history_${user.id}`;
    const savedMessages = localStorage.getItem(storageKey);
    
    // Nếu có dữ liệu cũ thì parse ra dùng, không thì trả về mảng rỗng
  ...
  // --- THAY ĐỔI 2: Tự động lưu vào LocalStorage khi messages thay đổi ---
  useEffect(() => {
    if (user?.id) {
      const storageKey = `chat_history_${user.id}`;
  ...
  // --- THAY ĐỔI 3: Thêm hàm xóa lịch sử (Tùy chọn) ---
  const clearHistory = () => {
    if (window.confirm("Bạn có chắc chắn muốn xóa toàn bộ lịch sử trò chuyện?")) {
      setMessages([]); // Xóa trên giao diện
      if (user?.id) {
        localStorage.removeItem(`chat_history_${user.id}`); // Xóa trong bộ nhớ
      }
    }
  };
                ...
                >
                  ...
                  >
                    ...
                    >
                      ...
                      >
                         {/* Xử lý hiển thị thời gian từ chuỗi JSON hoặc Date object */}
