...
const NewDashboard = () => {
  ...
  const { data: stats, isLoading: isLoadingStats } = useQuery(
    ...
    async () => {
      const [reminders, metrics] = await Promise.all([
        api.get("/health/reminders/today"),
  ...
  const CircularProgress = ({ percentage, value, unit, title, icon: Icon }) => {
    ...
    return (
      <div
        className={`p-6 flex flex-col rounded-xl shadow-lg border hover:shadow-2xl hover:-translate-y-1 transition-all duration-300 ${
          theme === "dark"
  ...
  const weeklyProgressPercentage = useMemo(() => {
    if (!fitnessData?.weeklyStats) return 0;
    const { completedDays, totalDays } = fitnessData.weeklyStats;
        ...
        <>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            ...
            >
              <CircularProgress
                ...
                value={fitnessData?.weeklyStats?.totalMinutes || 0}
                unit={t("minutes") || "min"}
            ...
            <motion.div variants={itemVariants}>
              <div
                className={`p-6 rounded-xl shadow-lg border h-full flex flex-col ${
                  theme === "dark"
                    ? "bg-[var(--glass-bg-primary)] border-[var(--glass-border)]"
                    : "bg-white border-gray-200"
                }`}
                ...
                <div className="space-y-2 flex-1">
                  {newsData && newsData.length > 0 ? (
                    newsData.map((article, index) => (
                      <div
                        key={index}
                        className={`p-2 rounded-lg shadow-sm hover:shadow-lg hover:-translate-y-1 transition-all duration-300 cursor-pointer ${
                          theme === "dark"
                            ? "bg-[var(--glass-bg-primary)] border-[var(--glass-border)]"
                            : "bg-white border-gray-200"
                        }`}
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                  ...
                  {filteredHistory.length === 0 ? (
                    ...
                    filteredHistory.map((h) => {
                      ...
                      return (
                        <div
                          key={h.id}
                          className={`p-4 rounded-lg hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 ${
                            theme === "dark"
                              ? "bg-[var(--glass-bg-primary)] border-[var(--glass-border)]"
                              : "bg-white border-gray-200"
                          }`}
            ...
            <div className="space-y-6">
              <motion.div variants={itemVariants}>
                <div
                  className={`p-6 rounded-xl shadow-lg border h-full hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 ${
                    theme === "dark"
                      ? "bg-[var(--glass-bg-primary)] border-[var(--glass-border)]"
                      : "bg-white border-gray-200"
                  }`}
                    ...
                    {todayReminders.length === 0 ? (
                      ...
                      todayReminders.map((r) => {
                        ...
                        return (
                          <div
                            key={r.id}
                            className={`p-3 rounded-lg hover:shadow-lg hover:-translate-y-1 transition-all duration-300 ${
                              theme === "dark"
                                ? "bg-[var(--glass-bg-primary)] border-[var(--glass-border)]"
                                : "bg-white border-gray-200"
                            }`}
                  ...
                  <div className="space-y-3">
                    <div className="flex items-center space-x-3">
                      <div className="text-xl">
                        {fitnessData?.todayWorkout?.thumbnail || "🧘‍♂️"}
                      </div>
                      <div className="flex-1">
                        <h3 className="text-sm font-semibold text-[var(--text-primary)] mb-1">
                          {fitnessData?.todayWorkout?.title}
                        </h3>
                        <div className="flex items-center space-x-3 text-xs text-[var(--text-secondary)]">
                          <span>
                            ⏱️ {fitnessData?.todayWorkout?.duration} phút
                          </span>
                          <span>
                            🔥 {fitnessData?.todayWorkout?.calories} cal
                          </span>
                        </div>
                      </div>
                      <span
                        className={`px-2 py-1 rounded-full text-xs font-medium ${
                          fitnessData?.todayWorkout?.completed
                    ...
                    <div className="space-y-1">
                      <div className="flex items-center justify-between text-xs">
                        ...
                        <span className="font-semibold text-[var(--text-primary)]">
                          {fitnessData?.weeklyStats?.completedDays}/
                          {fitnessData?.weeklyStats?.totalDays} ngày
                        </span>
                      </div>
                      <div
