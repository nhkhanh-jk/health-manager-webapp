...
const MedicalHistoryPage = () => {
  ...
  const submit = async (e) => {
    ...
    try {
      if (form.id) {
        await axios.put(`/health/history/${form.id}`, {
          date: form.date,
          title: form.title,
          notes: form.notes,
        });
      } else {
        await axios.post("/health/history", {
          date: form.date,
          title: form.title,
          notes: form.notes,
        });
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-3xl font-bold text-[var(--primary-600)]">
              {stats.total}
            </span>
          </div>
        ...
        <div className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-3xl font-bold text-[var(--accent-600)]">
              {stats.completed}
            </span>
          </div>
          <h3 className="text-lg font-semibold text-[var(--neutral-800)]">
